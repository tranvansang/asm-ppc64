...
.macro inc reg_name amount=1
	addi \reg_name, \reg_name, \amount
.endm

.macro dec reg_name amount=1
	subi \reg_name, \reg_name, \amount
.endm
...
#this macro will backup lr and restore after calling. It also setup frame for the call
#this is should be applied for single call (the caller only use only one subroutine once)
...
.macro backup_param from=1 to=-1
	assign_param_reg reg, \from
	assign_param_on_stack st, \from
	std reg, st(sp)
	.if \to + 1
		.if \from-\to
			backup_param "(\from + 1)", \to
		.endif
	.endif
.endm

.macro backup_param_while_calling from=1 to=-1
	assign_param_reg reg, \from
	assign_param_on_stack st, \from
	std reg, (st + frame_size)sp)
	.if \to + 1
		.if \from-\to
			backup_param_while_calling "(\from + 1)", \to
		.endif
	.endif
.endm

.macro restore_param from=1 to=-1
	assign_param_reg reg, \from
	assign_param_on_stack st, \from
	ld reg, st(sp)
	.if \to + 1
		.if \from-\to
			restore_param "(\from + 1)", \to
		.endif
	.endif
.endm

.macro restore_param_while_calling from=1 to=-1
	assign_param_reg reg, \from
	assign_param_on_stack st, \from
	ld reg, (st + frame_size)(sp)
	.if \to + 1
		.if \from-\to
			restore_param_while_calling "(\from + 1)", \to
		.endif
	.endif
.endm
